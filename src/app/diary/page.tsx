...
const generateDiceBearAvatar = (seed: string) => {
  // Using "shapes" style for more abstract avatars
...
function FamilyMemberForm({ member, onSave, onClose }: FamilyMemberFormProps) {
  ...
  const handleSubmit = () => {
    ...
    // If the URL is empty or still the placeholder, generate a DiceBear avatar.
    ...
    if (member && !trimmedCustomName) { // If editing and custom name cleared
      memberPayload.customName = null; 
    } else if (trimmedCustomName) { // If custom name has value
      memberPayload.customName = trimmedCustomName;
    }
    // If it's a new member and customName is empty, it will be omitted from memberPayload

    onSave(memberPayload, member?.id);
  };

  useEffect(() => {
    // This effect updates the avatar preview in the form if realName changes
    // and the current avatarUrl is either a placeholder or a DiceBear URL.
    const isCurrentUrlAutoGenerated = !avatarUrl || avatarUrl === DEFAULT_PLACEHOLDER_AVATAR || avatarUrl.startsWith('https://api.dicebear.com');

    if (isCurrentUrlAutoGenerated) {
      if (realName.trim()) {
        setAvatarUrl(generateDiceBearAvatar(realName.trim()));
      } else {
        // If realName is cleared and avatar was auto-generated, clear avatarUrl or set to placeholder
        setAvatarUrl(''); 
      }
    }
  }, [realName, avatarUrl]); // Rerun if realName or avatarUrl (if manually changed) changes
        ...
        <div className="grid grid-cols-4 items-center gap-4">
         ...
         <div className="grid grid-cols-4 items-center gap-4">
          ...
          <Input
            ...
            placeholder="Leave blank or placeholder for an auto-generated avatar"
          />
        </div>
...
const FamilyMemberDisplayCard = memo(({ member, onSelectMember, onEditMember, onDeleteMemberTrigger }: FamilyMemberDisplayCardProps) => {
  ...
  // If avatarUrl is valid, not a placeholder, and not a DiceBear URL, use it. Otherwise, generate a "shapes" avatar.
  const isCustomNonDiceBearUrl = member.avatarUrl &&
                                 member.avatarUrl !== DEFAULT_PLACEHOLDER_AVATAR &&
                                 !member.avatarUrl.includes('api.dicebear.com');

  const displayAvatarUrl = isCustomNonDiceBearUrl
    ? member.avatarUrl
            ...
            <div className="flex items-center gap-2 sm:gap-3">
              ...
              <div className="flex items-center gap-2">
                <Avatar className="h-8 w-8 sm:h-10 sm:w-10">
                  <AvatarImage
                    src={
                       (selectedMember.avatarUrl && selectedMember.avatarUrl !== DEFAULT_PLACEHOLDER_AVATAR && !selectedMember.avatarUrl.includes('api.dicebear.com'))
